...
func Encode(rand io.Reader, privateKeys []interface{}, certificates []*x509.Certificate, caCerts []*x509.Certificate, password string) (pfxData []byte, err error) {
	return LegacyRC2.WithRand(rand).Encode(privateKeys, certificates, caCerts, password)
}
...
func (enc *Encoder) Encode(privateKeys []interface{}, certificates []*x509.Certificate, caCerts []*x509.Certificate, password string) (pfxData []byte, err error) {
	...
	if authenticatedSafe[1], err = makeSafeContents(enc.rand, keyBags, nil, nil, 0, 0); err != nil {
		return nil, err
	}
